...
## Нафигация по репозиторю 
Финальный вариант построения обучающей выборки и обучения модели содержится в файле /transactions_xgboost.ipynb
Имплементация сервиса находится в директории /service
Исследования, проводимые в ходе работы содержатся в директории /research
Библиотеки необходимые для запуска файлов лежат в файле /regs.txt в корне проекта
Докер-файл для запуска приложения вместе с бд и хранилищем находится в корне проекта -  /docker-compose.yml
Инструкцию по запуску сервиса находит по пути /service/Readme.md
